...
class PortalDocenteController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        // ── Bloque "Hoy" ────────────────────────────────────────────────
        // Reutiliza datos ya cargados arriba (asignaciones, gridHorario,
        // franjasHorario, rendimiento) — solo agrega lo que no existía:
        // horario/próxima clase de hoy, conteo de asistencia pendiente,
        // conteo de entregas ZuraClass sin revisar, y la PlanifUnidad
        // vigente por asignación (única fuente de "planificación del día"
        // aprobada para esta versión — ver docs/ZURAEDU_ROLE_EXPERIENCE.md).
        ...
        $asignacionesSinNotas = collect($rendimiento)
            ->filter(fn($r) => ($r['sin_nota'] ?? 0) > 0)
            ->map(function ($r, $asigId) use ($asignaciones) {
                return [
                    'asignacion' => $asignaciones->firstWhere('id', $asigId),
                    'sin_nota'   => $r['sin_nota'],
                ];
            })
            ->values();
        ...
        return view('portal.docente.dashboard', compact(
            ...
            'diaHoy', 'horarioHoy', 'proximaClase', 'asignacionesHoy',
    ...
    private function buildMinerdDataDocente(?Matricula $matricula, ?SchoolYear $schoolYear, ?Docente $docente): ?array
    {
        ...
        return (new \App\Services\MinerdBoletinService())->build($matricula, $schoolYear, $docente);
    }
