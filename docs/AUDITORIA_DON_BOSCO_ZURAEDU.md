...
De los 31 requerimientos de Don Bosco: **13 🟢 existen y funcionan**, **9 🟡 existen parcialmente**, **4 🟠 existen pero están defectuosos**, **4 🔴 no existen**, **1 ⚫ requiere decisión institucional** (ver tabla completa en la sección final). **Actualización 2026-08-26/27: las recomendaciones #1 a #8 de §16 (numeración de recomendaciones) ya fueron implementadas y verificadas — #22, #7, #15, #17, #1, #26, #29, #9, #10, #31 y #12 pasan a 🟢, quedando 24/5/0/1/1. La recomendación §16.8 (test de regresión de cierre de año) además destapó y corrigió un bug crítico activo: el cierre de año fallaba siempre por un valor de ENUM inválido en `alertas_sistema.tipo` — ver actualización en §5. Se amplió después la cobertura de tests a calificaciones/boletines/RBAC (59 tests en total), completando el punto #12 de la tabla de 31. Se verificó el punto #8 de la tabla de 31 (optimización de calificaciones): el índice compuesto ya existía, confirmado con EXPLAIN — sin cambios de código, quedando 25/4/0/1/1. Se implementó el punto #20 (bloqueo o alerta de notas): el usuario decidió bloqueo real, opcional por institución — quedando 26/4/0/1/0, sin ningún punto pendiente de decisión institucional. Se implementó el punto #2 (Nivel): se descubrió que nivel↔ciclo no es una función fija en los datos reales (mismo nivel, distinto ciclo según la convención de nombres de grado del tenant) — en vez de forzar un mapeo que habría roto datos, se quitó la fragilidad real (ENUM de MySQL → VARCHAR + `Grado::CICLOS`), quedando 27/3/0/1/0. Finalmente se implementó el punto #25 (saldos financieros): se agregó un saldo consolidado (deuda de colegiatura + saldo de cafetería, mostrados juntos sin sumarse — son conceptos opuestos) en el expediente del estudiante, la página de cuenta y el PDF de estado de cuenta, quedando 28/2/0/1/0. Ver actualización en §5 para el detalle de las tres últimas.**
...
**#2 Nivel — ✅ RESUELTO (2026-08-27).** Ver actualización más abajo.
...
**#20 Bloqueo o alerta de notas — RESUELTO. Decisión institucional: bloqueo real, opcional por institución.** Antes solo existía una alerta informativa en `admin/boletines/ver.blade.php` que explícitamente "nunca impide consultar ni imprimir el boletín" — el único punto de la auditoría marcado ⚫ (requiere decisión de negocio, no código). El usuario decidió: bloqueo real, pero solo de impresión/exportación (nunca de la consulta en pantalla), con excepción para Administrador/Director, y apagado por defecto (cada institución lo activa si quiere).
- **Config nueva:** `boletines_config.bloquear_por_deuda` (migración `2026_08_27_000001`, boolean, default `false`), con su toggle en Config. Boletín (`admin/boletines/config.blade.php`, tarjeta "Control Financiero").
- **Dónde bloquea:** `BoletinController::pdf()`, `pdfAnual()` y `zipGrupo()` — nunca `verEstudiante()` (ver en pantalla siempre funciona, incluso con deuda y el toggle activado). En `zipGrupo()` el bloqueo es por estudiante: los que tienen deuda se omiten del ZIP (con aviso de cuántos y por qué), no se bloquea la exportación completa del grupo por un solo estudiante moroso.
- **Excepción:** Administrador/Director pueden forzar con `?forzar=1` — los botones de imprimir en `ver.blade.php` y en `grupo.blade.php` cambian a un ícono de candado abierto para ellos cuando hay bloqueo activo; para el resto de roles, esos mismos botones aparecen deshabilitados con un tooltip explicando el motivo.
- **Regla de negocio explícita:** un rol sin permiso de excepción que intenta `?forzar=1` igual queda bloqueado — el parámetro por sí solo no basta, se valida también el rol.
- Verificado con `tests/Feature/BoletinBloqueoDeudaTest.php` (10 pruebas: toggle apagado no cambia el comportamiento previo, toggle encendido bloquea individual/anual, ver() nunca se bloquea, sin deuda no bloquea, Administrador sin forzar también queda bloqueado, con forzar sí puede, un rol sin excepción no puede forzar, el ZIP omite solo al estudiante con deuda y con forzar los incluye a todos) y en navegador real contra datos reales de la demo (activé el toggle, confirmé el banner rojo y los candados, confirmé el bloqueo real de la descarga y el bypass con `forzar=1`, y restauré el toggle a apagado al terminar). Suite completa: 69/69 (59 previos + 10 nuevos).

**#2 Nivel — RESUELTO, con un hallazgo que cambió el enfoque original.** Antes de escribir código se inspeccionaron los datos reales de `grados.nivel`/`grados.ciclo` en todos los tenants: **`nivel↔ciclo` no es una función fija**, contra lo que asumía implícitamente la propuesta original de "normalizar la relación". El mismo `nivel=4` es `primer_ciclo` bajo la convención "Básica" (1ro-8vo grado) y `segundo_ciclo` bajo la convención "Secundaria" (1ro-6to grado) — ambas conviven en el sistema según cómo cada institución nombra sus grados. Forzar un mapeo rígido nivel→ciclo habría roto datos reales existentes. Se lo planteé al usuario, quien decidió: quitar la fragilidad real (el ENUM de MySQL), sin inventar una relación que no existe.
- **Migración `2026_08_27_000002`:** `grados.ciclo` pasó de `ENUM('inicial','primer_ciclo','segundo_ciclo','bachillerato')` a `VARCHAR(30)` (vía `DB::statement` con SQL crudo — el proyecto no tiene `doctrine/dbal` instalado, requerido por el `->change()` nativo de Laravel para alterar tipos de columna). Sin pérdida de datos: los 56 grados existentes conservaron su valor de `ciclo` intacto, verificado antes y después de la migración.
- **`Grado::CICLOS`** (nueva constante pública) es ahora la única fuente de verdad de los valores permitidos — agregar un ciclo nuevo en el futuro es un cambio de una línea en el modelo, sin ninguna migración. `OnboardingWizardController::guardarPaso3()` (única validación de `ciclo` de grado alimentada por input de usuario) se actualizó para derivar su regla de esa constante (`Rule::in(array_diff(Grado::CICLOS, ['inicial']))` — excluye `'inicial'` a propósito, esos grados los gestiona SuperAdmin, no el wizard).
- Se documentó explícitamente en `Grado.php` (PHPDoc) que `nivel` es un ordinal propio de cada institución y `ciclo` es la fuente de verdad real para agrupar — para que nadie intente derivar uno del otro en el futuro.
- Verificado: los 56 grados reales conservan su `ciclo` tras la migración; creación de grados con `ciclo='bachillerato'` e `'inicial'` probada en una transacción revertida. Suite completa: 69/69 (sin tests nuevos — cambio de infraestructura de datos, cubierto indirectamente por los tests existentes que ya ejercitan `Grado::scopePrimerCiclo()`/`scopeSegundoCiclo()`).

**#25 Saldos financieros — RESUELTO.** Antes cada módulo financiero mostraba su propio número por separado (`Pago` = deuda de colegiatura, `VentaCafeteria::saldoEstudiante()` = saldo prepago de cafetería) sin ningún lugar que los mostrara juntos. Son conceptos opuestos (deuda vs. crédito) — el "saldo consolidado" no los suma en un solo número (sería contable y visualmente engañoso), los muestra **juntos, uno al lado del otro**, en los 3 lugares donde un administrativo revisa la situación financiera de un estudiante:
- `admin/perfiles/estudiante.blade.php` (pestaña "Pagos" del expediente del estudiante): línea de resumen ahora incluye "Saldo Cafetería" junto a Cobrado/Pendiente (solo si el estudiante tiene saldo > 0, para no ensuciar la vista de instituciones sin cafetería).
- `admin.pagos.por-estudiante` (página "Estado de Cuenta" completa): nuevo chip "Saldo Cafetería" junto a Total Pagado/Por Pagar/Total Registros.
- `admin.pagos.estado-cuenta-pdf`: mismo chip nuevo en el PDF descargable, visible solo si el saldo es mayor a cero.
- Los tres puntos llaman al mismo `VentaCafeteria::saldoEstudiante($estudianteId)` ya existente — no se duplicó lógica de cálculo.
- Verificado en navegador real: se creó una recarga de cafetería de prueba (RD$500) para un estudiante real de la demo con deuda de colegiatura real, se confirmó que los 3 lugares muestran ambos números correctamente (incluyendo el contenido del PDF, verificado renderizando la plantilla directamente), y se eliminó el registro de prueba y se restauró el flag del módulo de Pagos a su estado original al terminar. Suite completa: 69/69 (sin tests nuevos — cambio de solo lectura sobre datos existentes, de bajo riesgo).
...
| 2 | Nivel | 🟢 Completo ✅ 2026-08-27 | Sí (como `ciclo`, ahora VARCHAR + `Grado::CICLOS`) | `Grado.php`, migración `2026_08_27_000002` | — | — |
...
| 20 | Bloqueo o alerta de notas | 🟢 Completo ✅ 2026-08-27 | Sí (bloqueo real, opcional) | `BoletinConfig::bloquear_por_deuda`, `BoletinController` | — | — |
...
| 25 | Saldos financieros | 🟢 Completo ✅ 2026-08-27 | Sí, consolidado (deuda + saldo cafetería) | `PagoController`, `perfiles/estudiante.blade.php` | — | — |
