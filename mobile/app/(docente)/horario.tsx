  ...
  return (
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      ...
      >
        ...
        {isLoading && <ActivityIndicator color={Colors.roles.docente} style={{ marginTop: 40 }} />}
        ...
        {clases.map((c: any, i: number) => (
          <View key={i} style={[styles.clase, { borderLeftColor: c.color ?? Colors.roles.docente }]}>
            <View style={[styles.claseLeft, { backgroundColor: (c.color ?? Colors.roles.docente) + '18' }]}>
              <Text style={[styles.hora, { color: c.color ?? Colors.roles.docente }]}>{c.franja?.inicio}</Text>
              <Text style={[styles.horaFin, { color: (c.color ?? Colors.roles.docente) + 'aa' }]}>{c.franja?.fin}</Text>
            </View>
            <View style={{ flex: 1, padding: 12 }}>
              <Text style={styles.materia}>{c.asignatura}</Text>
              {!!c.grupo && <Text style={styles.grupo}>{c.grupo}</Text>}
        ...
        {!isLoading && data?.publicado && clases.length === 0 && (
          <Text style={styles.empty}>No tienes clases este día.</Text>
        )}
      </ScrollView>
    </SafeAreaView>
  )
...
const styles = StyleSheet.create({
  ...
  dayBtnActive: { backgroundColor: Colors.roles.docente, borderColor: Colors.roles.docente },
  ...
  grupo:        { fontSize: 12, fontWeight: '600', color: Colors.roles.docente, marginTop: 2 },
