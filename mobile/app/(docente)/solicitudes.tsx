...
const EMPTY_FORM = { tipo: '', asunto: '', descripcion: '', fecha_inicio: '', fecha_fin: '' }

// ── Helpers de presentación ────────────────────────────────────────────────

function tipoIcon(tipo: string): string {
  const map: Record<string, string> = {
    permiso_dia:         'calendar-outline',
    licencia_enfermedad: 'medkit-outline',
    licencia_maternidad: 'heart-outline',
    constancia_trabajo:  'document-text-outline',
    constancia_salario:  'cash-outline',
    actualizacion_datos: 'create-outline',
    capacitacion:        'school-outline',
    cambio_horario:      'time-outline',
    otro:                'ellipsis-horizontal-circle-outline',
  }
  return map[tipo] ?? 'document-outline'
}

function formatFechaRango(inicio: string | null, fin: string | null): string | null {
  if (!inicio) return null
  const fmt = (d: string) => new Date(d).toLocaleDateString('es-DO', { day: '2-digit', month: 'short' })
  if (!fin || inicio === fin) return fmt(inicio)
...
export default function SolicitudesDocente() {
  ...
  const { data, isLoading, refetch, isRefetching } = useQuery({
    queryKey: ['solicitudes-docente'],
    queryFn:  () => solicitudesApi.index().then(r => r.data),
  })

  const { data: detailData, isLoading: detailLoading } = useQuery({
    queryKey: ['solicitud-doc', detailId],
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['solicitudes-docente'] })
  ...
  const tieneRango = (tipo: string) =>
    ['permiso_dia', 'licencia_enfermedad', 'licencia_maternidad', 'capacitacion'].includes(tipo)
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      ...
      >
        ...
        {solicitudes.length === 0 ? (
          ...
          const rango = formatFechaRango(sol.fecha_inicio, sol.fecha_fin)
          return (
              ...
              <View style={styles.cardMeta}>
                {rango && (
                  <View style={styles.metaChip}>
                    ...
                    <Text style={styles.metaText}>{rango}</Text>
                  </View>
                )}
                <View style={styles.metaChip}>
                ...
                {sol.respuesta && (
                  <View style={[styles.metaChip, { backgroundColor: Colors.green + '14', borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2 }]}>
                    <Ionicons name="chatbubble-outline" size={12} color={Colors.green} />
      ...
      <Modal visible={createVisible} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => { setCreateVisible(false); setForm(EMPTY_FORM) }}>
        <SafeAreaView style={styles.safe}>
          <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
            ...
            <ScrollView contentContainerStyle={styles.modalContent} keyboardShouldPersistTaps="handled">
              ...
              <View>
                ...
                <View style={styles.tiposGrid}>
                  {Object.entries(tipos).map(([key, label]: [string, any]) => (
                    ...
                    >
                      <Ionicons name={tipoIcon(key) as any} size={13} color={form.tipo === key ? ACCENT : Colors.muted} style={{ marginRight: 4 }} />
              ...
              {/* Rango de fechas (solo para tipos que lo requieren) */}
              {tieneRango(form.tipo) && (
                <View style={styles.fechaRangoRow}>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.fieldLabel}>Fecha inicio <Text style={styles.optional}>(opcional)</Text></Text>
                    <TextInput
                      style={styles.input}
                      placeholder="AAAA-MM-DD"
                      placeholderTextColor={Colors.muted}
                      value={form.fecha_inicio}
                      onChangeText={t => setForm(f => ({ ...f, fecha_inicio: t }))}
                    />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.fieldLabel}>Fecha fin <Text style={styles.optional}>(opcional)</Text></Text>
                    <TextInput
                      style={styles.input}
                      placeholder="AAAA-MM-DD"
                      placeholderTextColor={Colors.muted}
                      value={form.fecha_fin}
                      onChangeText={t => setForm(f => ({ ...f, fecha_fin: t }))}
                    />
                  </View>
                </View>
              )}
      ...
      {/* ── Modal detalle ─────────────────────────────────────────────────── */}
      <Modal visible={!!detailId} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setDetailId(null)}>
        <SafeAreaView style={styles.safe}>
          <View style={styles.modalHeader}>
          ...
          {detailLoading || !detail ? (
            ...
            <ScrollView contentContainerStyle={styles.modalContent}>
              ...
              <View style={styles.metaRow}>
                {formatFechaRango(detail.fecha_inicio, detail.fecha_fin) && (
                  <View style={styles.metaChip}>
                    <Ionicons name="calendar-outline" size={13} color={Colors.muted} />
                    <Text style={styles.metaText}>{formatFechaRango(detail.fecha_inicio, detail.fecha_fin)}</Text>
                  </View>
                )}
                <View style={styles.metaChip}>
                  ...
                  <View style={[styles.respuestaBox, { borderLeftColor: detail.estado_color }]}>
                    ...
                    {detail.respondido_en && (
                      <Text style={[styles.metaText, { marginTop: 8 }]}>
                        Respondido: {new Date(detail.respondido_en).toLocaleDateString('es-DO', { day: '2-digit', month: 'long', year: 'numeric' })}
                      </Text>
                    )}
                  </View>
                </>
              ...
              {!detail.respuesta && detail.estado === 'pendiente' && (
                <View style={styles.pendienteBox}>
                  <Ionicons name="hourglass-outline" size={20} color={Colors.amber} />
                  <Text style={[styles.metaText, { color: Colors.amber }]}>En espera de respuesta de administración</Text>
                </View>
              )}
...
const styles = StyleSheet.create({
  ...
  tiposGrid:     { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 2 },
  ...
  fechaRangoRow: { flexDirection: 'row', gap: 10 },
  ...
  respuestaBox:  { borderLeftWidth: 3, borderRadius: 10, backgroundColor: Colors.green + '08',
                   padding: 14, gap: 4 },
  pendienteBox:  { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: Colors.amber + '12',
