...
export default function CarnetEstudiante() {
  const router = useRouter()

  const { data, isLoading, isError, refetch, isRefetching } = useQuery({
    queryKey: ['carnet-estudiante'],
    queryFn:  () => carnetApi.miCarnet().then(r => r.data),
    staleTime: 300_000,
  })
      ...
      <SafeAreaView style={styles.safe} edges={['bottom']}>
        <View style={styles.centered}>
          ...
          <Text style={styles.emptyText}>
            Tu carnet digital aún no ha sido generado. Contacta a administración.
          </Text>
        </View>
      </SafeAreaView>
    )
  ...
  const { carnet, risk } = data
  const estado       = ESTADO_BADGE[carnet.estado] ?? { label: carnet.estado, color: Colors.muted }
  const qrImageUrl   = `https://quickchart.io/qr?text=${encodeURIComponent(carnet.qr_url)}&size=260&ecLevel=M&margin=2`
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      ...
      >
          ...
          <View style={styles.carnetBody}>
            ...
            <View style={styles.qrCol}>
              <Image
                ...
                resizeMode="contain"
              />
              <Text style={styles.qrHint}>Mostrar en portería</Text>
            </View>
          </View>
        ...
        <TouchableOpacity
          ...
          onPress={() => router.push('/(estudiante)/carnet-historial')}
          activeOpacity={0.85}
        >
