...
export default function ResultadosEvaluacionEstudiante() {
  const [expandedId, setExpandedId] = useState<number | null>(null)
  const [filtroAsig, setFiltroAsig] = useState<string | null>(null)

  const { data, isLoading, refetch, isRefetching } = useQuery({
    queryKey: ['resultados-estudiante'],
    queryFn: () => resultadosApi.index().then(r => r.data),
  })
  ...
  const filtrados = filtroAsig
    ? resultados.filter((r: any) => r.asignatura === filtroAsig)
    : resultados
    ...
    <SafeAreaView style={styles.safe} edges={['bottom']}>
      ...
      >
        <View style={styles.pageHeader}>
          <Text style={styles.pageTitle}>Mis Evaluaciones</Text>
            ...
            {asignaturas.map(asig => {
              ...
              return (
                <TouchableOpacity
                  ...
                  onPress={() => setFiltroAsig(active ? null : asig)}
                >
                  <Text style={[styles.filtroTxt, active && styles.filtroTxtActive]}>{asig}</Text>
                </TouchableOpacity>
              )
            })}
          </ScrollView>
                ...
                >
                  ...
                  <View style={styles.scoreCol}>
                    {res.ponderacion != null && (
                      <Text style={[styles.scoreNum, { color: res.asignatura_color }]}>
                        {res.ponderacion}
                      </Text>
                    )}
                    {nivelCfg && (
