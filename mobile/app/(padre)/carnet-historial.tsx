...
export default function CarnetHistorialPadre() {
  const { estudianteId } = useLocalSearchParams<{ estudianteId: string }>()
  const id = Number(estudianteId ?? 0)

  const { data, isLoading, isRefetching, refetch } = useQuery({
    queryKey: ['carnet-historial-hijo', id],
    queryFn:  () => carnetApi.historialHijo(id).then(r => r.data),
    enabled:  id > 0,
    staleTime: 60_000,
  })
      ...
      >
        ...
        {!isLoading && accesos.length === 0 && (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Las entradas y salidas de tu hijo/a aparecerán aquí.</Text>
          </View>
        )}
