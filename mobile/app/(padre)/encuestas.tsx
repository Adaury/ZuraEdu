...
export default function EncuestasPadre() {
  ...
  const enviar = useMutation({
    ...
    onSuccess: () => {
      ...
      setRespuestas({})
      qc.invalidateQueries({ queryKey: ['encuestas-padre'] })
    },
  ...
  const handleEnviar = () => {
    ...
    if (faltantes.length > 0) {
      Alert.alert('Respuestas incompletas', `Faltan ${faltantes.length} pregunta${faltantes.length > 1 ? 's' : ''} por responder.`)
      return
    }
      ...
      >
        ...
        {encuestas.map((e: any) => (
          <View key={e.id} style={[styles.card, { borderLeftColor: e.ya_respondio ? Colors.green : ACCENT }]}>
            <View style={{ flex: 1 }}>
              ...
              <View style={styles.chipRow}>
                <View style={[styles.chip, { backgroundColor: `${ACCENT}20` }]}>
                  <Text style={[styles.chipText, { color: ACCENT }]}>{e.preguntas_count} pregunta{e.preguntas_count !== 1 ? 's' : ''}</Text>
                </View>
                {!!e.fecha_cierre && (
            ...
            {!e.ya_respondio && (
              <TouchableOpacity style={[styles.btnResponder, { backgroundColor: ACCENT }]} onPress={() => abrirEncuesta(e)}>
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
          ...
          {cargandoDetalle ? (
            ...
            <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
              ...
              {detalle.preguntas.map((p: any, idx: number) => (
                <View key={p.id} style={styles.preguntaCard}>
                  ...
                  {p.tipo === 'opcion_multiple' && p.opciones.map((o: any) => (
                    <TouchableOpacity
                      key={o.id}
                      style={[styles.opcion, respuestas[p.id]?.opcion_id === o.id && { borderColor: ACCENT, backgroundColor: `${ACCENT}10` }]}
              ...
              <TouchableOpacity
                style={[styles.btnEnviar, { backgroundColor: ACCENT }, enviar.isPending && { opacity: 0.6 }]}
...
const styles = StyleSheet.create({
  ...
  btnResponder: { borderRadius: 8, paddingHorizontal: 12, paddingVertical: 7,
  ...
  btnEnviar:    { borderRadius: 12, padding: 16, alignItems: 'center', justifyContent: 'center', marginTop: 8 },
})
