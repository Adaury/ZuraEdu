...
import { gamificacionApi, classroomApi } from '../../services/api'
...
export default function MisPuntosPadre() {
  ...
  const { data, isLoading, refetch, isRefetching } = useQuery({
    queryKey: ['padre-puntos', hijoActual?.estudiante_id],
    queryFn:  () => gamificacionApi.hijo(hijoActual.estudiante_id).then(r => r.data),
      ...
      >
        ...
        {!hijoActual ? (
          <View style={styles.section}>
            <Text style={styles.empty}>Cargando datos del hijo/a...</Text>
          </View>
        ) : (
          <>
            {/* Stats */}
            ...
            {ranking.length > 0 && (
              <View style={styles.section}>
                ...
                {ranking.map((item: any) => (
                  <View key={item.posicion} style={[styles.rankRow, item.es_hijo && styles.rankRowHijo]}>
                    <Text style={styles.rankMedalla}>{MEDALLA(item.posicion)}</Text>
                    <Text style={[styles.rankNombre, item.es_hijo && { color: '#4338ca', fontWeight: '800' }]} numberOfLines={1}>
                      {item.nombre}{item.es_hijo ? ' (tu hijo/a)' : ''}
                    </Text>
                    <Text style={[styles.rankPts, item.es_hijo && { color: '#4338ca' }]}>{item.total} pts</Text>
                  </View>
                ))}
              </View>
            )}
...
const styles = StyleSheet.create({
  ...
  headerBanner:  { flexDirection: 'row', alignItems: 'center', gap: 12, borderRadius: 16, padding: 16 },
  headerTitle:   { fontSize: 18, fontWeight: '900', color: '#fff' },
  headerSub:     { fontSize: 12, color: 'rgba(255,255,255,.75)', marginTop: 2 },
  hijoPill:      { paddingHorizontal: 14, paddingVertical: 7, borderRadius: 99, backgroundColor: '#fff', borderWidth: 1.5, borderColor: Colors.border },
  ...
  rankRowHijo:   { backgroundColor: '#eef2ff', borderRadius: 10, paddingHorizontal: 8, borderBottomWidth: 0 },
