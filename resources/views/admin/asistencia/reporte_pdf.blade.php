...
<style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .header h1 { font-size:12.5pt; font-weight:bold; margin-bottom:2px; }
    .header p  { font-size:8pt; opacity:.85; }
    .info { font-size:8pt; color:#374151; margin-bottom:10px; }
    ...
    tbody tr.alerta { background:#fff1f2; }
    tbody tr:nth-child(even):not(.alerta) { background:#f8fafc; }
...
<div class="header">
    ...
    <p>{{ $asignacion->asignatura?->nombre }} &nbsp;·&nbsp; {{ $asignacion->grupo?->nombre_completo }} &nbsp;·&nbsp; {{ $schoolYear?->nombre }}</p>
</div>

<div class="info">
    <span><strong>Docente:</strong> {{ $asignacion->docente?->nombre_completo ?? '—' }}</span>
    <span><strong>Grupo:</strong> {{ $asignacion->grupo?->grado?->nombre }} {{ $asignacion->grupo?->seccion?->nombre }}</span>
...
<table>
    ...
    <tr class="{{ $s['alerta'] ? 'alerta' : '' }}">
        ...
        <td class="nombre">{{ $s['matricula']?->estudiante?->apellidos }}, {{ $s['matricula']?->estudiante?->nombres }}</td>
        <td>{{ $s['total'] }}</td>
