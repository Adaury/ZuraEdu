...
<head>
<meta charset="UTF-8">
<title>Eventos Extracurriculares</title>
<style>
...
<div class="hdr">
    ...
    <div class="hdr-body">
        ...
        <div>
            ...
            <div class="inst-sub">Eventos Extracurriculares</div>
        </div>
    </div>
</div>

<div class="doc-title">Lista de Eventos</div>
<div class="doc-meta">Generado el {{ now()->format('d/m/Y H:i') }} — Total: {{ $eventos->count() }} eventos</div>
...
<table>
    ...
    <tbody>
    @php $tiposLabels = ['academico'=>'Académico','deportivo'=>'Deportivo','cultural'=>'Cultural','social'=>'Social','otro'=>'Otro']; @endphp
    @forelse($eventos as $i => $evt)
    <tr>
        <td>{{ $i + 1 }}</td>
        <td><strong>{{ $evt->nombre }}</strong></td>
        <td>{{ $tiposLabels[$evt->tipo] ?? $evt->tipo }}</td>
        <td>{{ $evt->fecha_inicio ? \Carbon\Carbon::parse($evt->fecha_inicio)->format('d/m/Y') : '—' }}</td>
        <td>{{ $evt->fecha_fin ? \Carbon\Carbon::parse($evt->fecha_fin)->format('d/m/Y') : '—' }}</td>
        <td>{{ $evt->lugar ?? '—' }}</td>
        <td style="text-align:center;">{{ $evt->inscripciones_count }}</td>
        <td><span class="badge {{ $evt->activo ? 'activo' : 'inactivo' }}">{{ $evt->activo ? 'Activo' : 'Inactivo' }}</span></td>
    </tr>
    @empty
    <tr>
