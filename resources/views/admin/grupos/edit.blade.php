    ...
    <div>
        ...
        <p class="text-muted mb-0 mt-1" style="font-size:.82rem;">
            ...
            <span class="info-pill ms-2">
                ...
                {{ $grupo->nombre_corto ?? ($grupo->grado->nombre . ' ' . $grupo->seccion->nombre) }}
            </span>
        </p>
    </div>
</div>
                    ...
                    <div class="col-12">
                        ...
                        <select name="school_year_id" id="school_year_id"
                            ...
                            @foreach($schoolYears as $sy)
                                <option value="{{ $sy->id }}"
                                    {{ old('school_year_id', $grupo->school_year_id) == $sy->id ? 'selected' : '' }}>
                                    {{ $sy->nombre }}
                        ...
                        <select name="seccion_id" id="seccion_id"
                            ...
                            @foreach($secciones as $seccion)
                                <option value="{{ $seccion->id }}"
                                    {{ old('seccion_id', $grupo->seccion_id) == $seccion->id ? 'selected' : '' }}>
                                    {{ $seccion->nombre }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="tutor_id" id="tutor_id"
                            ...
                            @foreach($docentes as $docente)
                                <option value="{{ $docente->id }}"
                                    {{ old('tutor_id', $grupo->tutor_docente_id) == $docente->id ? 'selected' : '' }}>
