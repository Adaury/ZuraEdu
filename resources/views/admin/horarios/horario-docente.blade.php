...
@section('page-title', 'Horario por Docente')
...
.selector-card label { font-size: .83rem; font-weight: 700; color: #374151; white-space: nowrap; }
.selector-card select { flex: 1; min-width: 220px; }
...
.mh-header h1 { font-size: 1.15rem; font-weight: 800; color: #fff; margin: 0 0 .2rem; }
.mh-header p  { font-size: .82rem; color: rgba(255,255,255,.72); margin: 0; }
.mh-stats { display: flex; gap: 1rem; margin-left: auto; flex-wrap: wrap; }
.mh-stat {
    text-align: center;
...
.mh-stat .n { font-size: 1.35rem; font-weight: 900; color: #fff; line-height: 1; }
.mh-stat .l { font-size: .62rem; color: rgba(255,255,255,.65); text-transform: uppercase; letter-spacing: .05em; }
...
.schedule-table th.th-franja { background: #312e81; width: 90px; font-size: .68rem; }
.schedule-table td { border: 1px solid #f1f5f9; vertical-align: top; padding: 0; min-height: 72px; }
.schedule-table td.td-franja {
    background: #f8f9fc; text-align: center; padding: .5rem .25rem;
...
.clase-card {
    height: 100%; min-height: 68px; padding: .5rem .6rem; border-radius: 8px;
    margin: 3px; position: relative; display: flex; flex-direction: column; gap: .2rem;
}
.clase-asignatura { font-size: .76rem; font-weight: 800; color: #fff; line-height: 1.2; }
.clase-grupo {
    font-size: .68rem; font-weight: 600; color: rgba(255,255,255,.85);
    display: flex; align-items: center; gap: .3rem;
}
.clase-aula {
    font-size: .63rem; color: rgba(255,255,255,.7);
    display: flex; align-items: center; gap: .25rem; margin-top: auto;
}

.recreo-row td {
    background: #fef9ec; text-align: center; padding: .4rem;
    font-size: .72rem; font-weight: 600; color: #92400e; letter-spacing: .04em;
}
.empty-cell { min-height: 68px; background: #fafbff; }

.day-summary { display: flex; gap: .75rem; margin-bottom: 1.25rem; flex-wrap: wrap; }
.day-card {
    flex: 1; min-width: 88px; background: #fff; border: 1px solid #e5e7eb;
    border-radius: 10px; text-align: center; padding: .7rem .5rem;
}
.day-card .day-name { font-size: .72rem; font-weight: 700; color: #6b7280; text-transform: uppercase; letter-spacing: .05em; }
.day-card .day-num  { font-size: 1.35rem; font-weight: 900; color: #312e81; line-height: 1.1; }
.day-card .day-label { font-size: .63rem; color: #9ca3af; }

.legend-wrap { display: flex; flex-wrap: wrap; gap: .65rem; margin-top: 1.25rem; }
.legend-item { display: flex; align-items: center; gap: .45rem; font-size: .78rem; color: #374151; }
...
<x-breadcrumb :items="[
    ...
    ['label' => 'Por Docente', 'url' => ''],
]" />

{{-- ── Selector de docente ────────────────────────────────────────── --}}
<form method="GET" action="{{ route('admin.horarios.horario-docente') }}" class="selector-card">
    <label for="selectDocente"><i class="bi bi-person-badge me-1"></i>Seleccionar docente:</label>
    <select name="docente_id" id="selectDocente" class="form-select form-select-sm"
            onchange="this.form.submit()">
        <option value="">— Elige un docente —</option>
        @foreach($docentes as $d)
            <option value="{{ $d->id }}" @selected($docente?->id === $d->id)>
                {{ $d->apellidos }}, {{ $d->nombres }}
            </option>
        @endforeach
    </select>
    @if($docente)
    <a href="{{ route('admin.horarios.horario-docente.pdf', ['docente_id' => $docente->id]) }}" target="_blank"
...
<div class="no-horario-card">
    ...
    <p>Elige un docente del selector superior para ver su horario semanal.</p>
</div>
@else

{{-- ── Header del docente seleccionado ──────────────────────────── --}}
<div class="mh-header">
    <div class="mh-avatar">{{ strtoupper(substr($docente->nombres ?? 'D', 0, 1)) }}</div>
    <div>
        <h1>{{ $docente->nombre_completo ?? ($docente->apellidos.', '.$docente->nombres) }}</h1>
        <p>
...
<div class="no-horario-card">
    ...
    @if(! $horarioActivo)
        ...
        <p>Aún no se ha publicado un horario para este año escolar.</p>
    @else
        <h4>{{ $docente->apellidos }}, {{ $docente->nombres }} no tiene clases en el horario</h4>
        <p>El horario está publicado pero este docente no tiene asignaciones registradas.</p>
    @endif
</div>
@else
...
@endif {{-- fin @if horario y detalles --}}
@endif {{-- fin @if $docente --}}
