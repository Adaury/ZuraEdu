...
<div class="d-flex align-items-center mb-4 gap-3">
    ...
    <h4 class="fw-bold mb-0" style="color:var(--primary)">
        <i class="bi bi-calendar3 me-2"></i>Editar Período: {{ $periodo->nombre }}
    </h4>
</div>
        ...
        <form method="POST" action="{{ route('admin.periodos.update', $periodo) }}">
            ...
            <div class="mb-3">
                ...
                <select name="school_year_id" class="form-select @error('school_year_id') is-invalid @enderror" required>
                    ...
                    <option value="{{ $sy->id }}"
                        {{ old('school_year_id', $periodo->school_year_id) == $sy->id ? 'selected' : '' }}>
            ...
            <div class="row g-3 mb-3">
                <div class="col-sm-4">
                    ...
                    <input type="number" name="numero" class="form-control @error('numero') is-invalid @enderror"
                           value="{{ old('numero', $periodo->numero) }}" min="1" max="12" required>
                    @error('numero')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-sm-8">
                    <label class="form-label fw-semibold">Nombre <span class="text-danger">*</span></label>
                    <input type="text" name="nombre" class="form-control @error('nombre') is-invalid @enderror"
                           value="{{ old('nombre', $periodo->nombre) }}" required>
                ...
                <div class="col-sm-6">
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" name="cerrado" id="cerrado" value="1"
                               {{ old('cerrado', $periodo->cerrado) ? 'checked' : '' }}>
