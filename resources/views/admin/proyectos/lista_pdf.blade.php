...
<head>
<meta charset="UTF-8">
<title>Proyectos Escolares</title>
<style>
...
.est-finalizado { background:#d1fae5; color:#065f46; }
.est-cancelado  { background:#fee2e2; color:#991b1b; }
.est-planificacion { background:#fef9c3; color:#92400e; }
...
<div class="hdr">
    ...
    <div class="hdr-body">
        ...
        <div>
            ...
            <div class="inst-sub">Proyectos Escolares</div>
        </div>
    </div>
</div>

<div class="doc-title">Lista de Proyectos</div>
<div class="doc-meta">Generado el {{ now()->format('d/m/Y H:i') }} — Total: {{ $proyectos->count() }} proyectos</div>
...
<table>
    ...
    <tbody>
    @forelse($proyectos as $i => $p)
    @php
        $estadoSlug = 'est-' . str_replace(' ', '_', $p->estado ?? 'planificacion');
        $integrantes = $p->integrantes->map(fn($int) => trim(($int->estudiante?->nombres ?? '') . ' ' . ($int->estudiante?->apellidos ?? '')))->filter()->implode(', ');
    @endphp
    <tr>
        <td>{{ $i + 1 }}</td>
        ...
        <td>{{ $p->area ?? '—' }}</td>
        <td>{{ $p->tutor?->name ?? '—' }}</td>
        <td><span class="badge {{ $estadoSlug }}">{{ ucfirst($p->estado ?? '—') }}</span></td>
        <td>{{ $p->fecha_inicio?->format('d/m/Y') ?? '—' }}</td>
        <td style="text-align:center;">{{ $p->fases->count() }} ({{ $p->fases->where('completada', true)->count() }} ✓)</td>
        <td style="font-size:6.5pt;">{{ $integrantes ?: '—' }}</td>
    </tr>
    @empty
    <tr>
