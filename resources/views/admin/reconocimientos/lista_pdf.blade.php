...
<div class="hdr">
    ...
    <div class="hdr-body">
        ...
        <div>
            ...
            <div class="inst-sub">Diplomas y Reconocimientos</div>
        </div>
    </div>
</div>

<div class="doc-title">Lista de Reconocimientos</div>
<div class="doc-meta">Generado el {{ now()->format('d/m/Y H:i') }} — Total: {{ $recs->count() }} registros</div>
...
<table>
    ...
    <tr>
        ...
        <td>{{ $r->tipo?->nombre ?? '—' }}</td>
        <td>{{ $r->titulo ?? '—' }}</td>
        <td><span class="badge {{ $r->entregado ? 'entregado' : 'pendiente' }}">{{ $r->entregado ? 'Entregado' : 'Pendiente' }}</span></td>
    </tr>
    @empty
    <tr>
