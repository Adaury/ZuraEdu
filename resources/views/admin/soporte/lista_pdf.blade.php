...
.est-abierto    { background:#dbeafe; color:#1e40af; }
.est-en_proceso { background:#fef9c3; color:#92400e; }
.est-resuelto   { background:#d1fae5; color:#065f46; }
.est-cerrado    { background:#f3f4f6; color:#374151; }
.pri-alta       { background:#fee2e2; color:#991b1b; }
.pri-media      { background:#fef9c3; color:#92400e; }
.pri-baja       { background:#d1fae5; color:#065f46; }
...
<div class="hdr">
    ...
    <div class="hdr-body">
        ...
        <div>
            ...
            <div class="inst-sub">Tickets de Soporte Interno</div>
        </div>
    </div>
</div>

<div class="doc-title">Tickets de Soporte</div>
<div class="doc-meta">Generado el {{ now()->format('d/m/Y H:i') }} — Total: {{ $tickets->count() }} tickets</div>
...
<table>
    ...
    @php
        $catLabels  = \App\Models\TicketSoporte::CATEGORIAS;
        $priLabels  = \App\Models\TicketSoporte::PRIORIDADES;
        $estLabels  = \App\Models\TicketSoporte::ESTADOS;
    @endphp
    <tr>
        <td>{{ $i + 1 }}</td>
        <td><strong>{{ $t->titulo }}</strong></td>
        <td>{{ $t->solicitante?->name ?? '—' }}</td>
        <td>{{ $catLabels[$t->categoria] ?? $t->categoria }}</td>
        <td><span class="badge pri-{{ $t->prioridad }}">{{ $priLabels[$t->prioridad] ?? $t->prioridad }}</span></td>
        <td><span class="badge est-{{ $t->estado }}">{{ $estLabels[$t->estado] ?? $t->estado }}</span></td>
