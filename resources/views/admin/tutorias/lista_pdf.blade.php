...
.hdr { border:2px solid #4338ca; border-radius:4px; margin-bottom:.8rem; overflow:hidden; }
.hdr-top { background:#4338ca; color:#fff; text-align:center; font-size:6pt; font-weight:700;
...
.logo-box { width:44px; height:44px; border-radius:5px; background:#4338ca; color:#fff;
...
.doc-title { text-align:center; font-size:11pt; font-weight:900; color:#4338ca;
...
<div class="hdr">
    ...
    <div class="hdr-body">
        ...
        <div>
            ...
            <div class="inst-sub">Asignaciones de Tutorías</div>
        </div>
    </div>
</div>

<div class="doc-title">Lista de Tutorías</div>
<div class="doc-meta">Generado el {{ now()->format('d/m/Y H:i') }} — Total: {{ $tutorias->count() }} registros</div>
...
<table>
    <thead>
        <tr>
            ...
            <th style="width:28%">Docente Tutor</th>
    ...
    <tbody>
    @forelse($tutorias as $i => $t)
    <tr>
        <td>{{ $i + 1 }}</td>
        <td><strong>{{ $t->grupo?->nombre_completo ?? '—' }}</strong></td>
        <td>{{ $t->docente?->nombre_completo ?? '—' }}</td>
        <td style="text-align:center;">{{ $t->sesiones->count() }}</td>
        <td>{{ $t->schoolYear?->nombre ?? '—' }}</td>
