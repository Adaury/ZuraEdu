...
@section('page-title', 'Editar Quiz — '.$material->titulo)
@section('content')
...
<div class="mb-4 d-flex align-items-center gap-3">
    ...
    <div>
        <h5 class="fw-bold mb-0">Editar Quiz</h5>
        <small class="text-muted">{{ $claseVirtual->nombre }} &bull; {{ $material->titulo }}</small>
    </div>
</div>

<form method="POST" action="{{ route('portal.docente.classroom.quiz.actualizar', [$claseVirtual, $material]) }}" id="formQuiz">
@csrf @method('PUT')
<div class="row g-4">
...
<div class="col-lg-8">
    ...
    <div class="card-body p-4">
        ...
        <div id="preguntasContainer">
            {{-- Preguntas existentes cargadas dinámicamente por JS --}}
        </div>

        <div id="sinPreguntas" class="text-center py-5 text-muted" style="display:none;">
...
<div class="card-body p-4">
    ...
    <div class="mb-3">
        ...
        <input type="number" name="duracion_minutos" class="form-control" min="1" max="300"
               placeholder="Sin límite" value="{{ $quiz->duracion_minutos }}">
    </div>
    ...
    <div class="mb-3">
        ...
        <select name="intentos_max" class="form-select">
            ...
            <option value="{{ $n }}" {{ $quiz->intentos_max == $n ? 'selected' : '' }}>
                {{ $n }} intento{{ $n > 1 ? 's' : '' }}
            </option>
            @endforeach
        </select>
    ...
    <div class="form-check form-switch mb-2">
        <input class="form-check-input" type="checkbox" name="autocorreccion" id="autocorreccion" value="1"
               {{ $quiz->autocorreccion ? 'checked' : '' }}>
        <label class="form-check-label small" for="autocorreccion">Autocorrección al finalizar</label>
    </div>
    <div class="form-check form-switch mb-2">
        <input class="form-check-input" type="checkbox" name="mostrar_respuestas" id="mostrar_respuestas" value="1"
               {{ $quiz->mostrar_respuestas ? 'checked' : '' }}>
    ...
    <div class="form-check form-switch mb-4">
        <input class="form-check-input" type="checkbox" name="aleatorizar_preguntas" id="aleatorizar" value="1"
               {{ $quiz->aleatorizar_preguntas ? 'checked' : '' }}>
...
{{-- Templates (reutilizados de quiz_crear) --}}
    ...
    <div class="card-body p-3">
        <div class="d-flex align-items-center gap-2 mb-2">
            ...
            <div class="ms-auto d-flex gap-1">
                <input type="number" name="REPLACE_puntos" class="form-control form-control-sm puntos-input" style="width:70px;" min="0.5" step="0.5" value="1">
...
const preguntasExistentes = @json($quiz->preguntas);
...
window.addEventListener('DOMContentLoaded', () => {
    preguntasExistentes.forEach(p => {
        ...
        document.getElementById('preguntasContainer').appendChild(div.firstElementChild);

        const bloque = document.getElementById('preguntasContainer').lastElementChild;

        // Rellenar enunciado y puntos
        bloque.querySelector('[name*="[enunciado]"]').value = p.enunciado;
        bloque.querySelector('[name*="[puntos]"]').value    = p.puntos;

        if (tipo === 'multiple' && p.opciones?.length) {
            p.opciones.forEach((op, oi) => {
                agregarOpcionCon(bloque, idx, oi, op.texto, op.es_correcta);
            });
        } else if (tipo === 'verdadero_falso' && p.opciones?.length) {
            const correcta = p.opciones.find(o => o.es_correcta);
            if (correcta) {
                const radios = bloque.querySelectorAll('input[type=radio]');
                radios.forEach(r => { r.checked = r.value === (correcta.texto === 'Verdadero' ? 'V' : 'F'); });
            }
        }
    });

    if (!preguntaIndex) document.getElementById('sinPreguntas').style.display = '';
    actualizarNumeros();
    actualizarPuntaje();
});
...
function agregarOpcionCon(bloque, pIdx, oIdx, texto, correcta) {
    const container = bloque.querySelector('.opciones-container');
    const div = document.createElement('div');
    div.className = 'd-flex align-items-center gap-2';
    div.innerHTML = `
        <input type="radio" name="preguntas[${pIdx}][opciones][${oIdx}][correcta]" value="1" class="form-check-input flex-shrink-0"
               style="cursor:pointer;" onchange="marcarCorrectaUnica(this)" ${correcta ? 'checked' : ''}>
        <input type="text" name="preguntas[${pIdx}][opciones][${oIdx}][texto]" class="form-control form-control-sm"
               placeholder="Opción..." value="${texto.replace(/"/g,'&quot;')}" required>
...
function agregarOpcion(btn, tipo) {
    ...
    div.innerHTML = `
        <input type="radio" name="preguntas[${pIdx}][opciones][${oIdx}][correcta]" value="1" class="form-check-input flex-shrink-0"
               style="cursor:pointer;" title="Marcar como correcta" onchange="marcarCorrectaUnica(this)">
...
function marcarCorrectaUnica(radio) {
    radio.closest('.opciones-container').querySelectorAll('input[type=radio]').forEach(r => {
        r.value = r === radio ? '1' : '0';
    });
}
...
function actualizarPuntaje() {
    let total = 0;
    document.querySelectorAll('.puntos-input').forEach(el => { total += parseFloat(el.value) || 0; });
