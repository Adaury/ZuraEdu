...
@section('activeKey', 'plan-evaluacion')

@push('styles')
<style>
.peval-materia {
    background: var(--prt-card-bg, #fff);
    border-radius: 12px;
    border: 1px solid var(--prt-border, #e2e8f0);
    margin-bottom: 1.25rem;
    overflow: hidden;
}
.peval-materia-hd {
    background: linear-gradient(90deg, rgba(30,58,138,.07) 0%, transparent 100%);
    border-bottom: 1px solid var(--prt-border, #e2e8f0);
    padding: .75rem 1rem;
...
.peval-cat-fill {
    height: 8px; border-radius: 99px; flex-shrink: 0;
    transition: width .3s;
}
.peval-inst-item {
    padding: .55rem 1rem;
    border-bottom: 1px solid var(--prt-border, #f1f5f9);
    display: flex; align-items: center; gap: .6rem;
    font-size: .8rem;
}
...
<div class="prt-card" style="text-align:center;padding:2.5rem;">
    ...
    <p style="color:#64748b;font-size:.88rem;margin:0;">
        Aún no hay planes de evaluación publicados para tu grupo.
    </p>
</div>
@else
...
<p style="font-size:.78rem;color:#64748b;margin-bottom:1rem;">
    ...
    Distribución de pesos por categoría que usa cada docente para calcular tu nota en cada período.
</p>
...
<div class="peval-materia">
    ...
    <div class="peval-materia-hd">
        ...
        <div style="flex:1;min-width:0;">
            <div style="font-weight:800;font-size:.88rem;color:var(--prt-text,#1e293b);">
    ...
    @if($plan)
    <div style="padding:.85rem 1rem;border-bottom:1px solid var(--prt-border,#f1f5f9);">
        ...
        <div class="peval-cat-bar">
            ...
            <div style="flex:1;background:#f1f5f9;border-radius:99px;height:8px;overflow:hidden;">
                <div class="peval-cat-fill" style="width:{{ $pct }}%;background:{{ $cat['color'] }};"></div>
            </div>
        ...
        <div style="margin-top:.6rem;background:#f8fafc;border-radius:8px;border:1px solid #e2e8f0;overflow:hidden;">
            ...
            <div class="peval-inst-item">
                ...
                <span style="flex:1;color:var(--prt-text,#1e293b);">{{ $inst->titulo }}</span>
