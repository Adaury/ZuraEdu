...
@section('page-title', 'Boletín — ' . ($estudiante->nombre_completo ?? ''))
@section('portal-name', 'Portal del Representante')

@section('sidebar')
    @include('portal.padre._sidebar', ['activeKey' => 'boletin'])
@endsection
...
@section('bottom-nav')
    ...
    <a href="{{ route('portal.padre.hijo.boletin', $estudiante) }}" class="prt-nav-item active">
...
.bol-hero-stats { display: flex; gap: 1.25rem; flex-wrap: wrap; margin-left: auto; }
...
.mat-name { font-size: .84rem; font-weight: 700; color: var(--prt-text); min-width: 120px; flex: 1; }
...
.sit-pill { font-size: .65rem; font-weight: 800; border-radius: 99px; padding: .15rem .5rem; }
...
.nota-bar { height: 100%; border-radius: 99px; }
...
{{-- ── Hero card ─────────────────────────────────────────────────── --}}
<div class="bol-hero">
    <div class="bol-hero-avatar"><i class="bi bi-person-fill"></i></div>
    <div>
...
<div class="bol-section">
    ...
    <div style="padding:.85rem 1rem;border-bottom:1px solid var(--prt-border);">

        {{-- Encabezado materia --}}
        <div class="mat-name" style="margin-bottom:.6rem;">{{ $asignatura->nombre ?? '—' }}</div>
        ...
        <div class="comp-block">
            <div class="comp-block-hd" style="background:{{ $comp['color'] }};">
                <i class="bi {{ $comp['icon'] }}"></i> C{{ $ci }} — {{ $comp['nombre'] }}
            </div>
            ...
            @php
                ...
                $cfVal  = $cfDyn[$ci][$p];   // CF calculado dinámicamente
                $hasRec = $pBase !== null && $pBase < 70 && $rVal !== null && $rVal > 0;
                $cfCls  = $cfVal !== null ? ($cfVal >= 70 ? 'cbt-ok' : 'cbt-mal') : 'cbt-nd';
            @endphp
            <tr>
                <td class="cbt-per">P{{ $p }}</td>
                <td class="cbt-p {{ $pBase !== null && $pBase < 70 ? 'cbt-warn' : '' }}">{{ $fmtB($pBase) }}</td>
                <td>@if($hasRec)<span class="cbt-rp">+{{ $fmtB($rVal) }}</span>@endif</td>
            ...
            <div class="comp-block-ft">
                ...
                <span class="cft-val {{ $prom !== null ? ($prom >= 70 ? 'cbt-ok' : 'cbt-mal') : 'cbt-nd' }}">{{ $fmtB($prom) }}</span>
            </div>
        </div>
        @endforeach
        </div>
        ...
        <div class="bol-nota-final-row">
            <span style="font-size:.78rem;font-weight:700;">
                <i class="bi bi-award-fill" style="color:#7c3aed;"></i> Nota Final — {{ $asignatura->nombre ?? '—' }}
            </span>
...
<div style="text-align:center;padding:3.5rem 1rem;color:#64748b;">
    ...
    <div style="font-size:.8rem;">Aún no hay calificaciones publicadas para este estudiante.</div>
</div>
@endif
