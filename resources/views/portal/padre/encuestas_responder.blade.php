...
@section('bottom-nav')
    ...
    <a href="{{ route('portal.padre.encuestas') }}" class="prt-nav-item active">
...
<form method="POST" action="{{ route('portal.padre.encuestas.guardar', $encuesta) }}">
    @csrf
                ...
                <div style="padding-left:2.1rem;">
                    <div style="display:flex;gap:.5rem;flex-wrap:wrap;">
                        @foreach([1,2,3,4,5] as $valor)
                            @php
                                $colores = [1=>'#ef4444',2=>'#f97316',3=>'#eab308',4=>'#84cc16',5=>'#22c55e'];
                            @endphp
                            <label style="display:flex;flex-direction:column;align-items:center;gap:.3rem;cursor:pointer;">
                                <input type="radio"
                                       ...
                                       value="{{ $valor }}"
                                       class="sr-only"
                                       onchange="this.closest('.escala-group-{{ $pregunta->id }}').querySelectorAll('.escala-btn').forEach(b=>b.style.borderColor='#e5e7eb');this.nextElementSibling.style.borderColor='{{ $colores[$valor] }}'">
                                <span class="escala-btn" style="width:2.5rem;height:2.5rem;border-radius:50%;border:2px solid #e5e7eb;display:flex;align-items:center;justify-content:center;font-size:.9rem;font-weight:800;color:#374151;background:#fff;transition:all .15s;">
                                    {{ $valor }}
                                </span>
                            </label>
                        @endforeach
